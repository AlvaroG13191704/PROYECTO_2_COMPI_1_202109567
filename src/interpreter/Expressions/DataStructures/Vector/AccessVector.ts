...
export class AccessVector implements Expression {
  ...
  getType(controller: Controller, ts: TableSymbol): type {
    ...
    if (exist_id != null) {
      ...
      controller.append(`El vector ${this.id} no fue declarado, linea ${this.line} y columna ${this.column}`,);
      return type.ERROR;
    }
  }
  ...
  getValue(controller: Controller, ts: TableSymbol) {
    ...
    //   controller.append(`La variable ${this.id} no es un vector, linea ${this.line} y columna ${ this.column}`,);
    //   return null;
    // }

    let valuesArray: [] = variable?.value;
    let typeArray: type = variable?.type.enumType;
    ...
    if (indexType != type.INTEGER) {
      controller.append(`El indice del vector ${this.id} no es un numero, linea ${this.line} y columna ${this.column}`,);
      return null;
    }
    // verify if the index is in the range of the vector
    if (indexValue < 0 || indexValue >= valuesArray.length) {
      controller.append(`El indice del vector ${this.id} no esta en el rango, linea ${this.line} y columna ${this.column}`,);
      return null;
    }
