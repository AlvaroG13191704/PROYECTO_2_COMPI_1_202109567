...
export class ModifyVector implements Instruction {
  public id: string;
  ...
  execute(controller: Controller, ts: TableSymbol) {
    // get the vector from the table of symbols
    const variable = ts.getSymbol(this.id);

    if (variable === null) {
      controller.append(`Error semantico, no se encontro el vector ${this.id}, liena ${this.line}, columna ${this.column}`);
      return;
    }else {
  ...
  goOver(): Node {
    let father = new Node("MODIFICAR VECTOR","");
